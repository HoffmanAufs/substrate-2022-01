# 生成默认的配置模板
./target/release/node-template build-spec --disable-default-bootnode --chain local > ./tmp/AuraSpec.json

# 根据模板，修改customSpec.json的aura, grandp内容，

# 产生启动时可用的Raw配置文件
./target/release/node-template build-spec --chain=customSpec.json --raw --disable-default-bootnode > ./tmp/RawAuraSpec.json
